...
import { DidixOperationsUpdateProductOrchestrationCreateModalComponent } from './create/create-modal.component';
...
export class ModalsService extends BaseModalsService implements IUpdateModal, IViewModal, ICreateModal {
	...
	openUpdateModal(dto: DidixOperationsLuProductResponseDto): Subject<DidixOperationsLuProductResponseDto> {
		const config = {
			initialState: {
		...
		this.bsModalRef = this.modalService.show(DidixOperationsUpdateProductOrchestrationCreateModalComponent, config);
		return this.bsModalRef.content.confirmed;
	}

	openCreateModal(dto: DidixOperationsLuProductResponseDto): Subject<DidixOperationsLuProductResponseDto> {
		const config = {
			initialState: {
